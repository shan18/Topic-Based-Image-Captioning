...
def load_split_data(input_path, split):
    ...
    return (train_img_ids, train_images, train_captions), (val_img_ids, val_images, val_captions), (test_img_ids, test_images, test_captions)
...
def process_data(model, data_type, filenames, captions, save_path, data_dir, batch_size):
...
def main(args):
    ...
    _, train_images, train_captions = train_data
    _, val_images, val_captions = val_data
    _, test_images, test_captions = test_data
    ...
    process_data(  # training data
        model, 'train', train_images, train_captions, args.save, args.root, args.batch_size
    )
    process_data(  # validation data
        model, 'val', val_images, val_captions, args.save, args.root, args.batch_size
    )
    process_data(  # test data
        model, 'test', test_images, test_captions, args.save, args.root, args.batch_size
    )
